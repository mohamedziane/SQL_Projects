...
SELECT COUNT(*) AS Facilities_Count_Free
FROM Facilities 
WHERE membercost = 0;
...
SELECT facid AS Facility_ID, name AS Facility_Name, membercost as Member_Cost, monthlymaintenance
FROM `Facilities`
...
SELECT DISTINCT CONCAT (m.surname, ' ', m.firstname) AS Member, f.name AS CourtName
...
SELECT 
	DISTINCT CONCAT (m.surname, ' ', m.firstname) AS Member, 
	f.name AS Facility, 
...
SELECT DISTINCT CONCAT (m.surname, ' ', m.firstname) AS Member, 
	Facility, 
